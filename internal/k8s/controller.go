...
func (lbc *LoadBalancerController) syncVirtualServer(task task) {
	...
	for _, vsrError := range vsrErrors {
		...
		if vsrError.VirtualServerRoute != nil {
			vsr := vsrError.VirtualServerRoute
			state := conf_v1.StateInvalid
			vsEventTitle := "Ignored"
			msg := fmt.Sprintf("Ignored by VirtualServer %v/%v: %v", vs.Namespace, vs.Name, vsrError.Error)

			lbc.recorder.Eventf(vsrError.VirtualServerRoute, api_v1.EventTypeWarning, vsEventTitle, msg)

			if lbc.reportVsVsrStatusEnabled() {
				virtualServersForVSR := findVirtualServersForVirtualServerRoute(lbc.getVirtualServers(), vsr)
				err = lbc.statusUpdater.UpdateVirtualServerRouteStatusWithReferencedBy(vsr, state, vsEventTitle, msg, virtualServersForVSR)
